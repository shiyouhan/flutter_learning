...
class BasicPainter extends CustomPainter {
  ...
  final Paint _paint = Paint()
      ...
      // ..colorFilter =
      //     ColorFilter.mode(Colors.blueAccent, BlendMode.exclusion) // 颜色渲染模式，是混合模式
      // ..maskFilter = MaskFilter.blur(BlurStyle.inner, 3.0) // 模糊遮罩效果
      // ..filterQuality = FilterQuality.high // 颜色渲染模式的质量
      // ..strokeCap =
      //     StrokeCap.round // 末端处理 round: 圆滑处理（延伸一小截圆形） square：延伸一小截矩形 butt: 不延伸
      // ..strokeJoin = StrokeJoin
      //     .round // 拐角类型，仅影响多边形连线 (Canvas.drawPath) round:圆角 miter:锐角 bevel:斜角
      ;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // canvas可以直接绘制颜色，画布上的所有颜色均会被混合叠加，可实现全局置换
    // canvas.drawColor(Colors.grey, BlendMode.color);

    // 绘制图像
    // 注意这的image是ui包里的，而非material包。留意image生成方式
    // offset：图像左上角点
    ...
    // 调用cubicTo前一般线moveTo(x0,y0)确定起始点。
    // 起点(x0,y0),终点(x3, y3)
  ...
  // 是否需要重绘。通常在当前实例和旧实例属性不一致时返回 true
  @override
